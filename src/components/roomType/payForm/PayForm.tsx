...
const PayForm: React.FC<IProps> = ({ resvContext }) => {
	const { payInfo, setPayInfo, houseData, customMsgs, bookerInfo, setBookerInfo } = resvContext;
	const { bookingPayInfo } = houseData;
	const { width } = useWindowSize();
	const { bankAccountInfo, payMethods } = bookingPayInfo;
	const { password, expireM, expireY, idNum, cardNum, paymethod, sender } = payInfo;
	...
	return (
		<div className="payForm">
			...
			>
				...
				<JDselect
					selectedOption={PAYMETHOD_FOR_BOOKER_OP.find((so) => so.value === payInfo.paymethod)}
					placeholder={' '}
					autoSize
					onChange={(op: any) => {
			...
			{bankAccountInfo && (
				<JDalign hide={isCardMode}>
					...
					>
						<JDtypho className="payForm__label">{LANG('accountHolder')}</JDtypho>
			...
			<JDalign hide={!isCardMode} grid>
				...
				{customMsgs.PayPrecaution && <JDtypho size="small">{'*' + customMsgs.PayPrecaution}</JDtypho>}
			</JDalign>
		</div>
	);
};
...
export default React.memo(
	PayForm,
	({ resvContext }, { resvContext: resvContext2 }) => resvContext.payInfo === resvContext2.payInfo
);
